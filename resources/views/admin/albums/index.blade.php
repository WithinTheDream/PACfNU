...
@if(session('success'))
    <div class="bg-green-50 text-green-700 p-4 rounded-xl mb-6 border border-green-100 font-bold">
        {{ session('success') }}
    </div>
@endif

<!-- FORM ATAS FULL WIDTH (RATA KANAN KIRI) -->
<div class="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-gray-100 mb-8 relative overflow-hidden">
    <!-- Hiasan Background Hijau Halus -->
        ...
        <form action="{{ route('albums.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5 mb-5">
                ...
                <div class="md:col-span-2">
                    <label class="block text-sm font-bold text-gray-700 mb-2">Deskripsi / Waktu / Keterangan Tambahan (Opsional)</label>
                    <input type="text" name="deskripsi" class="w-full border border-gray-300 bg-gray-50 rounded-xl p-3.5 focus:ring-2 focus:bg-white focus:ring-green-200 focus:border-[#00923F] outline-none transition" placeholder="Contoh: Dilaksanakan pada 20 April 2026 di Gedung MWCNU Tahunan">
                </div>
            </div>
...
<!-- LIST ALBUM DI BAWAHNYA DENGAN PREVIEW FOTO -->
<div class="space-y-6">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex flex-col md:flex-row gap-6 hover:border-[#00923F] transition duration-300 group">
            ...
            <div class="md:w-1/3 flex flex-col justify-between">
                ...
                <div class="flex gap-3 mt-4">
                    ...
                    <!-- Tombol Hapus Album -->
                    <form action="{{ route('albums.destroy', $album->id) }}" method="POST" class="inline" onsubmit="return confirm('Yakin hapus album ini beserta SEMUA foto di dalamnya?')">
                        @csrf @method('DELETE')
                        <button class="px-4 py-2.5 bg-red-50 text-red-600 rounded-xl hover:bg-red-600 hover:text-white transition font-bold text-sm border border-red-100">
                            Hapus
                        </button>
                    </form>
                </div>
            ...
            <!-- Bagian Kanan: Preview Berjajar (Maks 4 Foto) -->
            <div class="md:w-2/3">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 h-full items-center">
                        @foreach($album->galleries->take(4) as $index => $foto)
                            <div class="relative rounded-xl overflow-hidden aspect-square border border-gray-100 shadow-sm group/img">
                                ...
                                <!-- Jika foto lebih dari 4, yang ke-4 dikasih overlay angka -->
